// -----------------------------------------------------------------------------
// apb_gpio_env.svh : top-level env for the apb_gpio IP.
//
//   Components:
//     m_apb_agent  : reused APB VIP agent (ACTIVE)  -> stimulus + monitor
//     m_gpio_agent : ACTIVE pin agent               -> drives gpio_in, monitors
//                                                      gpio_out/gpio_oe/irq
//     m_ref_model  : cycle-exact predictor (samples apb_if + gpio_if directly)
//     m_scoreboard : reference-model scoreboard (APB + PIN, paired fifos)
//     m_coverage   : gpio functional coverage
...
//     "env.m_ref_model"  "pin_vif"  (virtual gpio_if)
//     "env.m_gpio_agent" "gpio_vif" (virtual gpio_if)
//     "env"              "env_cfg"  (apb_gpio_env_cfg)
// -----------------------------------------------------------------------------
`ifndef APB_GPIO_ENV_SVH
`define APB_GPIO_ENV_SVH

class apb_gpio_env extends uvm_env;
  `uvm_component_utils(apb_gpio_env)

  apb_gpio_env_cfg               m_cfg;

  apb_agent                      m_apb_agent;
  gpio_agent                     m_gpio_agent;
  apb_gpio_ref_model             m_ref_model;
  apb_gpio_scoreboard            m_scoreboard;
  apb_gpio_coverage              m_coverage;

  uvm_reg_predictor #(apb_seq_item) m_predictor;
  apb_reg_adapter                m_adapter;

  function new(string name = "apb_gpio_env", uvm_component parent = null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(apb_gpio_env_cfg)::get(this, "", "env_cfg", m_cfg))
      `uvm_fatal("ENV_NOCFG", "apb_gpio_env_cfg 'env_cfg' not set")
    ...
    m_ref_model  = apb_gpio_ref_model::type_id::create("m_ref_model", this);
    ...
    if (m_cfg.en_scoreboard)
      m_scoreboard = apb_gpio_scoreboard::type_id::create("m_scoreboard", this);

    if (m_cfg.en_coverage)
      m_coverage = apb_gpio_coverage::type_id::create("m_coverage", this);
  endfunction
  ...
  function void connect_phase(uvm_phase phase);
    ...
    if (m_cfg.en_scoreboard) begin
      ...
      m_ref_model.pin_exp_ap.connect(m_scoreboard.pin_exp_fifo.analysis_export);
      m_gpio_agent.mon.ap.connect(m_scoreboard.pin_act_fifo.analysis_export);
    end

    // ---- coverage : APB stream + reference-model gpio events ----
